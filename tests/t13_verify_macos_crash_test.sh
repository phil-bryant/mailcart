#!/usr/bin/env bash
# Self-contained mailcart test lane (mailcart-owned; wraps the Makefile PLCrashReporter verification target).
...
#R010: Build the UI bundle under the macOS build lock, then run the Makefile PLCrashReporter verification target.
