...
# Thin-pointer contract checks for the mailcart t03 static-security lane wrapper.
...
setup() {
  #R001: Test harness setup for t03_run_static_security_tests contract checks.
  REPO_ROOT="$(mailcart_repo_root)"
  SRC="${REPO_ROOT}/tests/t03_run_static_security_tests.sh"
}
...
@test "delegates to the mapped runner golden" {
  #R015-T01: Verify the pointer calls delegate_golden for the static-security golden with "$@".
  run grep 'delegate_golden "tests/t03_run_static_security_tests.sh" "$@"' "${SRC}"
  [ "$status" -eq 0 ]
}
