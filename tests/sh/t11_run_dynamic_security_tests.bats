...
# Thin-pointer contract checks for the mailcart t11 dynamic-security lane wrapper.
...
@test "delegates to the mapped runner golden" {
  #R015-T01: Verify the pointer calls delegate_golden for the dynamic-security golden with "$@".
  run grep 'delegate_golden "tests/t09_run_dynamic_security_tests.sh" "$@"' "${SRC}"
  [ "$status" -eq 0 ]
}
