...
Numbered runbook scripts (`00`-`05`, `11`) and `tests/tNN_*` lanes are thin pointers that set
`RUNBOOK_REPO_ROOT`, source `runner/config/runbook/mailcart.env`, and exec the corresponding golden in
[`../runner/`](../runner/). `05_install_matchy_api_tls.sh` generates local TLS material for the HTTPS API.
