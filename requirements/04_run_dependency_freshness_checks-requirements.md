# 04 Run Dependency Freshness Checks Wrapper Requirements
...
Design: Use `exec "${RUNNER_HOME}/04_run_dependency_freshness_checks.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `04_run_dependency_freshness_checks.sh` with `"$@"`.
