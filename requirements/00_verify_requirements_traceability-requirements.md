...
Design: Infer test files from requirements path and scoped source conventions, including `tests/sh`, `tests/py`, top-level `Makefile` mapping to `tests/sh/Makefile.bats`, and Swift tests in `macos-ui/Tests` and `macos-ui/UITests`, while canonicalizing symlinked test paths.
Tests:
