# 00 Verify Requirements Traceability Wrapper Requirements
...
Design: Use `exec "${RUNNER_HOME}/00_verify_requirements_traceability.sh" "$@"` so arguments pass through unchanged.
Tests:
- R015-T01: Verify wrapper source delegates to `00_verify_requirements_traceability.sh` with `"$@"`.
