# t16 run graph bridge replay tests Lane Requirements

## Scope

Applies to `tests/t16_run_graph_bridge_replay_tests.sh`.
...
R010  Statement: Lane runs the deterministic Graph bridge replay Makefile target from repo root.
Design: `cd "$RUNBOOK_REPO_ROOT"` and `exec make _graph-replay-test`.
Tests:
