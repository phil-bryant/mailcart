...
Design: Call `delegate_golden "src/scripts/install_api_tls_generic.sh" "$@"` so the shim execs `${RUNNER_HOME}/src/scripts/install_api_tls_generic.sh` with arguments passed through unchanged.
Tests:
- R015-T01: Verify the pointer calls `delegate_golden "src/scripts/install_api_tls_generic.sh"` with `"$@"`.
